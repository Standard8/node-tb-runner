...
/**
 * Takes a path to a binary file (like `/Applications/ThunderbirdNightly.app`)
...
function normalizeBinary (binaryPath, platform, arch) {
  return when.promise(function(resolve, reject) {
    ...
    arch = arch || os.arch();
    binaryPath = binaryPath || process.env.JPM_THUNDERBIRD_BINARY || "thunderbird";
    ...
    if (platform === "osx") {
      ...
      binaryPath = isAppPath ? path.join(binaryPath, "Contents/MacOS/thunderbird") :
                   binaryPath;
    ...
    function fallBack () {
      ...
      if (arch === "(64)") {
        console.warn("You are using 32-bit version of Thunderbird on 64-bit versions of the Windows.\nSome features may not work correctly in this version. You should upgrade Thunderbird to the latest 64-bit version now!")
        programFilesVar = "ProgramFiles(x86)";
      }
      resolve(path.join(process.env[programFilesVar], appName, "thunderbird.exe"));
    }
...
normalizeBinary.paths = {
  "thunderbird on osx": "/Applications/Thunderbird.app/Contents/MacOS/thunderbird",
  "beta on osx": "/Applications/ThunderbirdBeta.app/Contents/MacOS/thunderbird",
  "nightly on osx": "/Applications/Daily.app/Contents/MacOS/thunderbird",
};

normalizeBinary.appNames = {
  "thunderbird on linux": "thunderbird",
  "beta on linux": "thunderbird-beta",
  "nightly on linux": "thunderbird-nightly",
  "thunderbird on windows": "Mozilla Thunderbird",
  // the default path in the beta installer is the same as the stable one
  "beta on windows": "Mozilla Thunderbird",
  "nightly on windows": "Daily"
};
...
function findMacAppByChannel(channel, opt) {
  ...
  var results = opt.spawnSync(
    "mdfind", ["kMDItemCFBundleIdentifier == 'org.mozilla." + macChannelMap[channel] + "'"]
  );
  var result = null;
  if (results.stdout) {
